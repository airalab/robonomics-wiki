...
У платы есть несколько входов/выходов - они выделены синим и зеленым цветом. Рассмотрим синий блок входов.
![plata1](../images/sensors-connectivity/plata1.png)

Слева направо(все выходы подписаны):
- вход для питания платы. рекомендуемое напряжение 12В. 
- Земля. служит как для подключения питания, так и для подключения датчиков.
- Питание датчиков. Выход, к которому подключается датчик/шина датчиков.
- выход SDA. служит для подключения датчиков по интерфейсу I2C.
- Выход SDL/1wire. Служит для подключения датчиков по интерфейсу I2C/1Wire.

Конфигурацию Датчиков, подключаемые к данному блоку выводов можно настраивать. Для этого используется блок переключателей, обозначенный желтым на изображении. 
Верхний переключатель нужен для переключения выходного напряжения на 3В или 5В. Нижний же переключатель нужен для переключения между интерфейсами.
